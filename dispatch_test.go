...
// the SDK decodes must resolve to exactly one disposition in the table, so
...
func TestEveryWireTypeHasADisposition(t *testing.T) {
	for wireType := range decodeRegistry {
		if _, ok := dispositionTable[wireType]; !ok {
			t.Errorf("decodable type %q has no disposition — the table must place it", wireType)
		}
	}
...
// asserts it maps to the concrete Event type the table prescribes.
//
...
// indistinguishable. gap's empty last_pos is deliberately NOT rejected: the contract
