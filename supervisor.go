...
func (c *Client) run(connectCtx context.Context) {
	...
	for {
		...
		// connected-path strikes during a backoff the contract says is non-terminal.
		c.setConn(nil)
...
func (c *Client) acquireConn(connectCtx context.Context, first bool) (Conn, error) {
	...
	// non-terminal forever (line 63), never a doomed handshake. Static
...
func (c *Client) dispatch(e *epoch, data []byte) {
	...
	if err != nil {
		...
		// the epoch-tear-on-non-JSON refinement (liveness) is a later task.
